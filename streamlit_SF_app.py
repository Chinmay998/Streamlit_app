...
# df_history = session.table("CLIMATE.HISTORY_DAY").filter(col('COUNTRY') == 'US').select(col('POSTAL_CODE'),col('MAX_TEMPERATURE_FEELSLIKE_2M_F'))
# df_history = df_history.group_by('POSTAL_CODE').max('MAX_TEMPERATURE_FEELSLIKE_2M_F').filter(col('POSTAL_CODE') != '445001').filter(col('DATE_VALID_STD')<= '2023-04-18').sort('MAX(MAX_TEMPERATURE_FEELSLIKE_2M_F)',ascending=False).limit(10).collect()
...
df_history = pd.read_sql ("SELECT  POSTAL_CODE,MAX(MAX_TEMPERATURE_FEELSLIKE_2M_F)MAX_TEMPERATURE_FEELSLIKE_2M_F,DATE_VALID_STD FROM CLIMATE.HISTORY_DAY where COUNTRY = 'US' AND DATE_VALID_STD <= '2023-04-18' group by POSTAL_CODE,DATE_VALID_STD limit 10",conn)
#st.write(df_history)

df_forecast =pd.read_sql ("SELECT  POSTAL_CODE,MAX(MAX_TEMPERATURE_FEELSLIKE_2M_F)MAX_TEMPERATURE_FEELSLIKE_2M_F,DATE_VALID_STD FROM GLOBAL_WEATHER_REPORT.CLIMATE.FORCAST_DAY where COUNTRY = 'US' AND DATE_VALID_STD <= '2023-04-18' group by POSTAL_CODE,DATE_VALID_STD limit 10",conn)
